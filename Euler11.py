string = """08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08<br />
...
inc = 0                                             # isolate numbers from the grid
while inc < len(string):
    if string[inc] == '<':
        ...
        grid.append([int(string[inc + 1: inc + 3])])
        inc += 3
    elif string[inc] == ' ':
        inc += 1
    else:
        grid[-1].append(int(string[inc: inc + 2]))
        inc += 2

highest = 0

for i in range(len(grid)):                             # using 4 separate loops
    for j in range(len(grid[0]) - 3):
        this_score = 1
        for k in range(4):
            this_score *= grid[i][j + k]
...
for i in range(len(grid)):                                  # in 4 times less loops (4 times faster)
    for j in range(len(grid[0])):
        this_score = [1] * 4
        for k in range(4):
            if i < len(grid) - 3:
                this_score[0] *= grid[i + k][j]
                if j < len(grid) - 3:
                    this_score[1] *= grid[i + k][j + k]
                if 3 <= j:
                    this_score[2] *= grid[i + k][j - k]
            if j < len(grid[0]) - 3:
                this_score[3] *= grid[i][j + k]
        for a in this_score:
            if a > high_score:
                high_score = a
